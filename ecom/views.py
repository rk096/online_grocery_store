...
def cart_view(request):
    ...
    if 'product_ids' in request.COOKIES:
        ...
        product_count_in_cart = 0
        return render(request, 'ecom/emptycart.html' , {'product_count_in_cart': product_count_in_cart})
    products = None
    total = 0
    if 'product_ids' in request.COOKIES:
        product_ids = request.COOKIES['product_ids']
...
def remove_from_cart_view(request, pk):
    ...
    if 'product_ids' in request.COOKIES:
        ...
        response = render(request, 'ecom/cart.html', {
                          'combined': combined, 'total': total, 'product_count_in_cart': product_count_in_cart})
        if value == "":
...
def send_feedback_view(request):
    ...
    if request.method == 'POST':
        ...
        if feedbackForm.is_valid():
            feedbackForm.save()
            return render(request, 'ecom/feedback_sent.html' , {'product_count_in_cart': product_count_in_cart})
